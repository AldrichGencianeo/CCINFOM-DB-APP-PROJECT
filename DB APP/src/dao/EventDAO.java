...
public class EventDAO {
    ...
    public void viewEventWithSchedules(int eventID) throws SQLException {
        ...
        if (event == null) {
            System.out.println("model.Event not found with ID: " + eventID);
            return;
        }
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            while (resultSet.next()) {
                ...
                System.out.printf("model.Schedule ID: %d | Date: %s | Time: %s - %s%n",
                        scheduleID, scheduleDate, startTime, endTime);

                if (sectionName != null) {
                    System.out.printf("  model.Section: %s | Price: %.2f | Capacity: %d | Available Slots: %s%n",
