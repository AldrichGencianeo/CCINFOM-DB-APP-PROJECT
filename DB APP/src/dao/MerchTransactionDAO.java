...
public class MerchTransactionDAO {
    ...
    public boolean checkStock(int merchandiseID, int quantity) throws SQLException {
        ...
        if (merch == null) {
            System.out.println("model.Merchandise not found.");
            return false;
        }
    ...
    public boolean processMerchTransaction(int customerID, int eventID, int merchandiseID, int quantity)
        ...
        try {
            Merchandise merch = merchandiseDAO.viewMerchandise(merchandiseID);
            if (merch == null) {
                System.out.println("Error: model.Merchandise not found.");
                connection.rollback();
                return false;
            }
            ...
            if (customer == null) {
                System.out.println("Error: model.Customer not found.");
                connection.rollback();
                return false;
            }

            System.out.println("\n=== TRANSACTION DETAILS ===");
            System.out.println("model.Customer: " + customer.getFirstName() + " " + customer.getLastName());
    ...
    public void viewCustomerReceipts(int customerID) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            while (resultSet.next()) {
                hasReceipts = true;
                System.out.printf("Receipt #%d | model.Event: %s | Item: %s | Qty: %d | Total: %.2f%n",
